let dataArt;
let mustangSvg;

function preload() {
  // Load the SVG file - make sure this file exists in your project folder
  mustangSvg = loadImage('mustang.svg');
}

function setup() {
  createCanvas(windowWidth, windowHeight);
  colorMode(RGB); // Switch to RGB for SMU brand colors
  textFont('Arial');
  frameRate(30);
  
  // Add a slight delay before initializing to ensure window size is correctly measured
  setTimeout(() => {
    // Initialize the visualization
    dataArt = new MustangsEngagementArt();
  }, 100);
}

function draw() {
  // No background here - we'll handle it in the drawBackgroundElements method
  
  if (dataArt) {
    dataArt.update();
    dataArt.display();
  }
}

function windowResized() {
  resizeCanvas(windowWidth, windowHeight);
  if (dataArt) {
    dataArt.resize();
  }
}

function mouseMoved() {
  if (dataArt) {
    dataArt.checkHover(mouseX, mouseY);
  }
}

class MustangsEngagementArt {
  constructor() {
    this.studentData = [
      { groupRange: "0-5", avgTotal: 2.2, avgActive: 1.6, activeRatio: 72.8 },
      { groupRange: "6-9", avgTotal: 7.3, avgActive: 5.9, activeRatio: 81.3 },
      { groupRange: "10-13", avgTotal: 11.3, avgActive: 9.8, activeRatio: 86.2 },
      { groupRange: "14-17", avgTotal: 15.2, avgActive: 13.9, activeRatio: 90.8 },
      { groupRange: "18-21", avgTotal: 19.3, avgActive: 17.0, activeRatio: 88.2 },
      { groupRange: "22-25", avgTotal: 23.2, avgActive: 21.3, activeRatio: 92.0 },
      { groupRange: "26-29", avgTotal: 28.1, avgActive: 25.2, activeRatio: 89.8 },
      { groupRange: "30-33", avgTotal: 31.2, avgActive: 27.0, activeRatio: 86.4 },
      { groupRange: "34-37", avgTotal: 37.0, avgActive: 31.0, activeRatio: 83.8 },
      { groupRange: "38-41", avgTotal: 39.2, avgActive: 35.2, activeRatio: 89.7 },
      { groupRange: "42-45", avgTotal: 43.0, avgActive: 39.0, activeRatio: 90.6 },
      { groupRange: "50-59", avgTotal: 50.0, avgActive: 45.0, activeRatio: 90.0 }
    ];
    
    // Filter out any empty ranges
    this.studentData = this.studentData.filter(item => item.avgTotal > 0);
    
    // SMU brand colors in RGB format
    this.smuBlue = [53, 76, 161];
    this.smuRed = [204, 0, 53]; 
    this.smuYellow = [249, 200, 14];
    this.smuTeal = [89, 195, 195];
    this.white = [255, 255, 255];
    
    // Color palette for mustangs based on activation ratio
    this.colorPalette = [
      this.white,      // Lowest activation (0%)
      this.white,      // Lower mid activation
      this.smuYellow,  // Higher mid activation
      this.smuRed      // Highest activation (100%+)
    ];
    
    this.mustangs = [];
    this.activeGroups = [];
    this.inactiveGroups = [];
    this.centerX = windowWidth / 2;
    this.centerY = windowHeight / 2;
    this.maxRadius = min(windowWidth, windowHeight) * 0.4;
    this.hoverMustang = null;
    this.time = 0;
    
    // Animation settings - smoother motion
    this.animationSpeed = 0.6; // Reduced for smoother motion
    
    this.initializeVisualization();
  }
  
  // Get color by interpolating through the palette based on activation ratio
  getColorFromPalette(ratio) {
    // Map the ratio to a position in our color palette
    const maxRatio = 100; // Maximum activation ratio (100%)
    
    // Adjust the mapping to ensure lower values stay white longer
    // Use a non-linear mapping to emphasize the white-to-color transition point
    let normalizedRatio;
    if (ratio < 30) {
      // Keep values below 30% as white or very close to white
      normalizedRatio = ratio / 120; // Very slow transition from white
    } else {
      // Transition more quickly after 30%
      normalizedRatio = 0.25 + ((ratio - 30) / 70) * 0.75;
    }
    
    normalizedRatio = constrain(normalizedRatio, 0, 1);
    
    // Calculate which segment of the palette we're in
    const segments = this.colorPalette.length - 1;
    const segment = normalizedRatio * segments;
    const index = floor(segment);
    const t = segment - index; // fractional part for interpolation
    
    if (index >= segments) {
      return this.colorPalette[segments];
    }
    
    // Linear interpolation between the two closest colors
    const colorA = this.colorPalette[index];
    const colorB = this.colorPalette[index + 1];
    
    return [
      lerp(colorA[0], colorB[0], t),
      lerp(colorA[1], colorB[1], t),
      lerp(colorA[2], colorB[2], t)
    ];
  }
  
  initializeVisualization() {
    // Create a grid layout for mustangs with adjusted spacing
    const rows = 3;
    const cols = 5;
    const totalMustangs = this.studentData.length;
    
    // Calculate header height based on window size (approximate)
    const headerHeight = windowHeight * 0.12; // Estimate header takes up about 12% of screen height
    
    // Calculate available space for visualization
    const availableHeight = windowHeight - headerHeight - (windowHeight * 0.10); // Subtracting header and 10% bottom margin
    
    // Calculate grid spacing, ensuring rows are closer but still don't overlap
    const gridSpacingX = windowWidth * 0.75 / (cols - 1); // Slightly reduced from 0.8
    const gridSpacingY = availableHeight * 0.65 / (rows - 1); // Reduced from 0.7 to bring rows closer
    
    const startX = this.centerX - (gridSpacingX * (cols - 1)) / 2;
    
    // Position the grid to start just below the header and ensure the bottom row fits
    const startY = headerHeight + (availableHeight * 0.2); // Start at header height plus a small padding
    
    // Create mustangs for each student group range
    for (let i = 0; i < totalMustangs; i++) {
      // Calculate grid position
      const row = Math.floor(i / cols);
      const col = i % cols;
      
      const x = startX + col * gridSpacingX;
      const y = startY + row * gridSpacingY;
      
      // Determine mustang size based on total group count
      const size = map(sqrt(this.studentData[i].avgTotal), 0, sqrt(50), 50, 130); // Adjusted size range
      
      // Get color based on activation ratio
      const colorRgb = this.getColorFromPalette(this.studentData[i].activeRatio);
      
      const mustang = {
        x: x,
        y: y,
        targetX: x,
        targetY: y,
        size: size,
        data: this.studentData[i],
        colorRgb: colorRgb,
        angle: 0,
        verticalOffset: 0, // For running animation - vertical component
        tiltAngle: 0,      // For running animation - rotational component
        gallopSpeed: random(0.04, 0.05) * this.animationSpeed, // Smoother animation
        hovering: false,
        row: row,
        col: col,
        // All mustangs facing same direction
        flipX: false
      };
      
      this.mustangs.push(mustang);
      
      // Create active student groups around this mustang
      const activeCount = ceil(mustang.data.avgActive);
      for (let j = 0; j < activeCount; j++) {
        const angle = random(TWO_PI);
        // Increased orbit radius for better spacing between mustangs' dots
        const radius = mustang.size * 1.2 * random(0.8, 1.2);
        const activeGroup = {
          x: mustang.x + cos(angle) * radius,
          y: mustang.y + sin(angle) * radius,
          size: 9, // Slightly larger dots
          parentMustang: mustang,
          colorRgb: this.smuRed.slice(), // Clone the color array
          opacity: j < mustang.data.avgActive ? 1 : 0.3, // Partial opacity for fractional counts
          angle: angle,
          distance: radius,
          speed: random(0.015, 0.025)
        };
        this.activeGroups.push(activeGroup);
      }
      
      // Create inactive student groups (limited for visual clarity)
      const inactiveCount = ceil(mustang.data.avgTotal - mustang.data.avgActive);
      const inactiveDisplayCount = min(inactiveCount, 40);
      
      for (let j = 0; j < inactiveDisplayCount; j++) {
        const angle = random(TWO_PI);
        // Decreased orbit radius to keep inactive dots closer to their mustang
        const radius = mustang.size * 0.9 * random(0.8, 1.2);
        const inactiveGroup = {
          x: mustang.x + cos(angle) * radius,
          y: mustang.y + sin(angle) * radius,
          size: 6, // Slightly larger dots
          parentMustang: mustang,
          colorRgb: this.smuTeal.slice(), // Clone the color array
          opacity: j < inactiveCount ? 0.6 : 0.2,
          angle: angle,
          distance: radius,
          speed: random(0.008, 0.012)
        };
        this.inactiveGroups.push(inactiveGroup);
      }
    }
  }
  
  resize() {
    this.centerX = windowWidth / 2;
    this.centerY = windowHeight / 2;
    
    // Recalculate grid layout with adjusted spacing
    const rows = 3;
    const cols = 5;
    
    // Calculate header height based on window size (approximate)
    const headerHeight = windowHeight * 0.12; // Estimate header takes up about 12% of screen height
    
    // Calculate available space for visualization
    const availableHeight = windowHeight - headerHeight - (windowHeight * 0.10); // Subtracting header and 10% bottom margin
    
    // Calculate grid spacing, ensuring rows are closer but still don't overlap
    const gridSpacingX = windowWidth * 0.75 / (cols - 1); // Slightly reduced from 0.8
    const gridSpacingY = availableHeight * 0.65 / (rows - 1); // Reduced from 0.7 to bring rows closer
    
    const startX = this.centerX - (gridSpacingX * (cols - 1)) / 2;
    
    // Position the grid to start just below the header and ensure the bottom row fits
    const startY = headerHeight + (availableHeight * 0.2); // Start at header height plus a small padding
    
    // Reposition mustangs
    for (let i = 0; i < this.mustangs.length; i++) {
      const mustang = this.mustangs[i];
      const row = mustang.row;
      const col = mustang.col;
      
      mustang.targetX = startX + col * gridSpacingX;
      mustang.targetY = startY + row * gridSpacingY;
    }
  }
  
  update() {
    this.time += 0.01;
    
    const isDimming = this.hoverMustang !== null;
    
    // Update mustangs
    for (let mustang of this.mustangs) {
      mustang.x = lerp(mustang.x, mustang.targetX, 0.1);
      mustang.y = lerp(mustang.y, mustang.targetY, 0.1);
      
      // Only animate if hovering
      if (mustang.hovering) {
        // More noticeable running animation
        const runCycle = this.time * 3.5; // Faster cycle
        
        // Vertical movement component - more pronounced up and down
        mustang.verticalOffset = sin(runCycle) * 5.0; // Increased amplitude
        
        // Forward tilt component - more noticeable forward/backward tilt
        mustang.tiltAngle = sin(runCycle - PI/6) * 0.04; // Increased tilt
      } else {
        // Reset animation when not hovering
        mustang.verticalOffset = 0;
        mustang.tiltAngle = 0;
      }
    }
    
    // Update active groups
    for (let group of this.activeGroups) {
      const parent = group.parentMustang;
      
      // Only move groups if their parent is not dimmed (either no mustang is hovered or this is the hovered mustang)
      if (!isDimming || parent === this.hoverMustang) {
        // Follow parent mustang with a slight delay
        group.angle += group.speed;
        group.x = parent.x + cos(group.angle) * group.distance;
        group.y = parent.y + sin(group.angle) * group.distance;
      }
    }
    
    // Update inactive groups
    for (let group of this.inactiveGroups) {
      const parent = group.parentMustang;
      
      // Only move groups if their parent is not dimmed (either no mustang is hovered or this is the hovered mustang)
      if (!isDimming || parent === this.hoverMustang) {
        // Slow movement around parent mustang
        group.angle += group.speed * 0.5;
        group.x = parent.x + cos(group.angle) * group.distance;
        group.y = parent.y + sin(group.angle) * group.distance;
      }
    }
  }
  
  display() {
    // Draw background elements
    this.drawBackgroundElements();
    
    // Check if we need to apply dimming effect
    const isDimming = this.hoverMustang !== null;
    
    // Draw inactive groups
    for (let group of this.inactiveGroups) {
      noStroke();
      
      // Apply dimming if needed
      if (isDimming && group.parentMustang !== this.hoverMustang) {
        // Dim inactive groups that don't belong to the hovered mustang
        fill(group.colorRgb[0], group.colorRgb[1], group.colorRgb[2], 255 * group.opacity * 0.3);
      } else {
        fill(group.colorRgb[0], group.colorRgb[1], group.colorRgb[2], 255 * group.opacity);
      }
      
      ellipse(group.x, group.y, group.size);
    }
    
    // Draw active groups
    for (let group of this.activeGroups) {
      noStroke();
      
      // Apply dimming if needed
      if (isDimming && group.parentMustang !== this.hoverMustang) {
        // Dim active groups that don't belong to the hovered mustang
        fill(group.colorRgb[0], group.colorRgb[1], group.colorRgb[2], 255 * group.opacity * 0.3);
        // Draw smaller glow for dimmed groups
        fill(group.colorRgb[0], group.colorRgb[1], group.colorRgb[2], 255 * group.opacity * 0.1);
        ellipse(group.x, group.y, group.size * 1.2);
      } else {
        fill(group.colorRgb[0], group.colorRgb[1], group.colorRgb[2], 255 * group.opacity);
        // Add glow to active groups
        fill(group.colorRgb[0], group.colorRgb[1], group.colorRgb[2], 255 * group.opacity * 0.3);
        ellipse(group.x, group.y, group.size * 1.5);
      }
      
      // Redraw the main circle to ensure proper layering
      if (isDimming && group.parentMustang !== this.hoverMustang) {
        fill(group.colorRgb[0], group.colorRgb[1], group.colorRgb[2], 255 * group.opacity * 0.3);
      } else {
        fill(group.colorRgb[0], group.colorRgb[1], group.colorRgb[2], 255 * group.opacity);
      }
      ellipse(group.x, group.y, group.size);
    }
    
    // Draw mustangs
    for (let mustang of this.mustangs) {
      // Draw dimmed mustangs first (if any hovered)
      if (isDimming && mustang !== this.hoverMustang) {
        this.drawMustangSvg(mustang, true); // true = dimmed
      }
    }
    
    // Draw the hovered mustang last (on top)
    if (this.hoverMustang) {
      this.drawMustangSvg(this.hoverMustang, false); // false = not dimmed
    }
    
    // If no mustang is hovered, draw all normally
    if (!isDimming) {
      for (let mustang of this.mustangs) {
        this.drawMustangSvg(mustang, false);
      }
    }
    
    // Draw hover information
    if (this.hoverMustang) {
      this.drawMustangInfo(this.hoverMustang);
    }
  }
  
  drawBackgroundElements() {
    // Create a gradient background from red to blue
    const gradientTop = color(204, 0, 53); // SMU Red at the top
    const gradientBottom = color(33, 46, 121); // Darker SMU Blue at the bottom
    
    // Draw the gradient
    for(let y = 0; y < height; y++) {
      const inter = map(y, 0, height, 0, 1);
      const c = lerpColor(gradientTop, gradientBottom, inter);
      stroke(c);
      line(0, y, width, y);
    }
    
    // Add subtle circular elements in background
    noStroke();
    for (let i = 0; i < 5; i++) {
      const circleSize = width * (0.1 + i * 0.05);
      const opacity = 5 - i; // Decreasing opacity for outer circles
      fill(255, 255, 255, opacity);
      ellipse(this.centerX, this.centerY, circleSize);
    }
  }
  
  drawMustangSvg(mustang, dimmed = false) {
    push();
    translate(mustang.x, mustang.y + mustang.verticalOffset); // Add vertical offset for running motion
    
    // Apply running animation tilt when hovering
    rotate(mustang.tiltAngle);
    
    // Flip horizontally if needed
    if (mustang.flipX) {
      scale(-1, 1);
    }
    
    // Apply the color tint using RGB colors
    if (dimmed) {
      // Dim the mustang
      tint(
        mustang.colorRgb[0] * 0.6,
        mustang.colorRgb[1] * 0.6,
        mustang.colorRgb[2] * 0.6,
        200 // Reduce opacity for dimmed mustangs
      );
    } else if (mustang.hovering) {
      // Make the mustang brighter when hovering
      tint(
        constrain(mustang.colorRgb[0] * 1.2, 0, 255),
        constrain(mustang.colorRgb[1] * 1.2, 0, 255),
        constrain(mustang.colorRgb[2] * 1.2, 0, 255)
      );
    } else {
      tint(mustang.colorRgb[0], mustang.colorRgb[1], mustang.colorRgb[2]);
    }
    
    // Calculate aspect ratio adjustment
    const targetWidth = mustang.size;
    const targetHeight = targetWidth * (mustangSvg.height / mustangSvg.width);
    
    // Draw the SVG image
    imageMode(CENTER);
    image(mustangSvg, 0, 0, targetWidth, targetHeight);
    
    // Remove tint
    noTint();
    
    pop();
    
    // Only display text information if this mustang is not hovering 
    // or if this is the currently hovered mustang
    if (!this.hoverMustang || mustang === this.hoverMustang) {
      // Add text for group count range directly below without box
      push();
      textAlign(CENTER, CENTER);
      textStyle(BOLD);
      
      // Draw white outline text for better visibility against any background
      let groupTextY = mustang.y + mustang.size * 0.6;
      
      // Draw text outline/shadow for better readability
      for (let i = -1; i <= 1; i++) {
        for (let j = -1; j <= 1; j++) {
          if (i !== 0 || j !== 0) { // Skip the center position
            fill(0, 0, 0, dimmed ? 50 : 150);
            textSize(mustang.size * 0.20); // Increased text size
            text(mustang.data.groupRange, mustang.x + i, groupTextY + j);
          }
        }
      }
      
      // Main text - group range
      fill(255, 255, 255, dimmed ? 150 : 255);
      textSize(mustang.size * 0.20); // Increased text size
      text(mustang.data.groupRange, mustang.x, groupTextY);
      
      // Activation percentage with similar outline technique
      let percentTextY = groupTextY + mustang.size * 0.22;
      textSize(mustang.size * 0.16); // Increased text size
      
      // Outline/shadow for percentage
      for (let i = -1; i <= 1; i++) {
        for (let j = -1; j <= 1; j++) {
          if (i !== 0 || j !== 0) {
            fill(0, 0, 0, dimmed ? 50 : 150);
            text(`${mustang.data.activeRatio.toFixed(1)}%`, mustang.x + i, percentTextY + j);
          }
        }
      }
      
      // Main text - percentage
      fill(
        this.smuYellow[0], 
        this.smuYellow[1], 
        this.smuYellow[2], 
        dimmed ? 180 : 255
      );
      text(`${mustang.data.activeRatio.toFixed(1)}%`, mustang.x, percentTextY);
      
      pop();
    }
  }
  
  drawMustangInfo(mustang) {
    const data = mustang.data;
    const infoX = mustang.x + mustang.size;
    const infoY = mustang.y - mustang.size * 0.5;
    
    // Draw info panel background with increased opacity
    fill(33, 46, 121, 245); // Darker SMU Blue background with 96% opacity
    stroke(204, 0, 53);
    strokeWeight(2);
    rectMode(CORNER);
    rect(infoX, infoY, 240, 140, 10); // Larger panel for bigger text
    
    // Draw info text - increased text size
    noStroke();
    textAlign(LEFT, TOP);
    fill(255);
    textSize(18); // Increased from 16
    text(`Group Range: ${data.groupRange}`, infoX + 12, infoY + 12);
    
    textSize(16); // Increased from 14
    fill(240);
    text(`Total Groups: ${data.avgTotal.toFixed(1)}`, infoX + 12, infoY + 42);
    
    // Use a more readable color for active groups (light yellow instead of red)
    fill(249, 220, 92); // Lighter yellow that's easier to read
    text(`Active Groups: ${data.avgActive.toFixed(1)}`, infoX + 12, infoY + 72);
    
    // Use SMU teal for inactive groups
    fill(89, 195, 195);
    text(`Inactive Groups: ${(data.avgTotal - data.avgActive).toFixed(1)}`, infoX + 12, infoY + 102);
  }
  
  checkHover(x, y) {
    let hoveredMustang = null;
    
    // Reset hover state
    for (let mustang of this.mustangs) {
      mustang.hovering = false;
    }
    
    // Check if mouse is over any mustang
    for (let mustang of this.mustangs) {
      const d = dist(x, y, mustang.x, mustang.y);
      if (d < mustang.size * 0.8) { // Increased hover detection area
        hoveredMustang = mustang;
        mustang.hovering = true;
        break;
      }
    }
    
    this.hoverMustang = hoveredMustang;
  }
}